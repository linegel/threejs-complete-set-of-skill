...
import { ARTIFACT_RELATIVE_DIR } from './artifact-config.js';
...
async function captureRgba( page, state ) {
	...
	const bytesPerRow = Number.isFinite( capture.bytesPerRow ) ? capture.bytesPerRow : capture.width * 4;

	if ( ! Number.isInteger( bytesPerRow ) || bytesPerRow < capture.width * 4 ) {
...
function sampleCapturePixel( capture, x, y ) {
	...
	const offset = sourceY * capture.bytesPerRow + x * 4;
...
async function captureImage( page, imagePath, state ) {
	...
	const png = createRgbaPng( capture.width, capture.height, ( x, y ) => {
		...
		return sampleCapturePixel( capture, x, y );
...
async function main() {
	...
	try {
		...
		if ( pageState.ready !== true ) {

			throw new Error( pageState.error ?? 'Image-pipeline validation page did not become ready.' );
		...
		const captures = [
			[ 'images/final.design.png', { mode: 'final', camera: 'design', seed: 'seed-image-pipeline-185', timeSeconds: 0, frame: 0 } ],
			[ 'images/no-post.design.png', { mode: 'no-post', camera: 'design', seed: 'seed-image-pipeline-185', timeSeconds: 0, frame: 0 } ],
			[ 'images/diagnostics.mosaic.png', { mode: 'diagnostics', camera: 'design', seed: 'seed-image-pipeline-185', timeSeconds: 0, frame: 0 } ],
			[ 'images/camera.near.png', { mode: 'final', camera: 'near', seed: 'seed-image-pipeline-185', timeSeconds: 0, frame: 0 } ],
			[ 'images/camera.design.png', { mode: 'final', camera: 'design', seed: 'seed-image-pipeline-185', timeSeconds: 0, frame: 0 } ],
			[ 'images/camera.far.png', { mode: 'final', camera: 'far', seed: 'seed-image-pipeline-185', timeSeconds: 0, frame: 0 } ],
			[ 'images/seed-0001.final.png', { mode: 'final', camera: 'design', seed: 'seed-0001', timeSeconds: 0.25, frame: 8 } ],
			[ 'images/seed-stress.final.png', { mode: 'final', camera: 'design', seed: 'seed-stress', timeSeconds: 1.1, frame: 33 } ],
			[ 'images/temporal.t000.png', { mode: 'final', camera: 'design', seed: 'seed-image-pipeline-185', timeSeconds: 0, frame: 0, staticControl: true } ],
			[ 'images/temporal.t001.png', { mode: 'final', camera: 'design', seed: 'seed-image-pipeline-185', timeSeconds: 1.6, frame: 48 } ],
			[ 'images/velocity.static.png', { mode: 'velocity', camera: 'design', seed: 'seed-image-pipeline-185', timeSeconds: 0, frame: 0, staticControl: true } ],
			[ 'images/velocity.motion.png', { mode: 'velocity', camera: 'design', seed: 'seed-image-pipeline-185', timeSeconds: 0.75, frame: 23 } ],
			[ 'images/AO.static.png', { mode: 'AO.r', camera: 'design', seed: 'seed-image-pipeline-185', timeSeconds: 0, frame: 0, staticControl: true } ],
			[ 'images/bloom.static.png', { mode: 'bloom contribution', camera: 'design', seed: 'seed-image-pipeline-185', timeSeconds: 0, frame: 0, staticControl: true } ],
			[ 'images/normal.static.png', { mode: 'normal', camera: 'design', seed: 'seed-image-pipeline-185', timeSeconds: 0, frame: 0, staticControl: true } ],
			[ 'images/albedo.static.png', { mode: 'albedo', camera: 'design', seed: 'seed-image-pipeline-185', timeSeconds: 0, frame: 0, staticControl: true } ]
		];

		for ( const [ imagePath, state ] of captures ) {

			if ( imagePath === 'images/diagnostics.mosaic.png' ) {

				await captureDiagnosticMosaic( page, imagePath, state );

			} else {

				await captureImage( page, imagePath, state );
		...
		console.log( `Image-pipeline evidence written to ${ artifactDir }` );
