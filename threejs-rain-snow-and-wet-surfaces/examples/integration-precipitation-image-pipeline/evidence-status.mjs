...
const sessionDir = resolve(
  process.env.LAB_ARTIFACT_DIR
    ?? resolve(root, "artifacts/visual-validation/integration-precipitation-image-pipeline/correctness"),
);
const labId = "integration-precipitation-image-pipeline";
...
if (operation !== "validate:artifacts" && operation !== "validate-artifacts") {
  throw new RangeError(`unknown precipitation integration evidence operation: ${operation}`);
}
