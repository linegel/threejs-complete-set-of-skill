import { TemporalSurfaceBrowserHost } from "./browser-host.js";
...
async function boot() {
  ...
  if (!canvas || !status) throw new Error("temporal-surface integration page is incomplete");
  const route = resolveTemporalIntegrationRoute(globalThis.location.href);
  const controller = new TemporalSurfaceBrowserHost({ canvas, route });
