...
function pushBranchVertex(
  ...
  buffers.childSlots.push(childSlot);
  buffers.angularSlots.push(angularSlot);
  buffers.barkUVChecker.push(barkUVChecker);
  buffers.windDisplacement.push(windDisplacement);
  return index;
}
...
function buildGeometry(buffers, includeContinuation = false) {
  ...
  if (includeContinuation) {
    ...
    geometry.setAttribute(
      "aChildSlot",
      new THREE.Float32BufferAttribute(buffers.childSlots, 1),
    );
    geometry.setAttribute(
      "aAngularSlot",
      new THREE.Float32BufferAttribute(buffers.angularSlots, 1),
    );
    geometry.setAttribute(
      "barkUVChecker",
      new THREE.Float32BufferAttribute(buffers.barkUVChecker, 1),
    );
    geometry.setAttribute(
      "windDisplacement",
      new THREE.Float32BufferAttribute(buffers.windDisplacement, 1),
    );
  } else if (buffers.leafRoots) {
    geometry.setAttribute(
      "leafRoot",
      new THREE.Float32BufferAttribute(buffers.leafRoots, 3),
    );
    geometry.setAttribute(
      "leafUvY",
    ...
    for (
      ...
      for (
        ...
        pushBranchVertex(
          ...
          branch.continuation,
          branch.childSlot ?? -1,
          branch.angularSlot ?? -1,
          (radialIndex + sectionIndex) % 2,
          0,
        );
      }
      pushBranchVertex(
...
export function reportAshForegroundBoundsAndFrustum(tree, camera = null) {
  ...
  const report = {
    ...
    leafBounds: copyBox3(leafBox),
    foregroundBounds: copyBox3(unionBox),
    frustum: {
      tested: false,
      containsForeground: null,
      cameraPosition: null,
    },
  };
  ...
  if (camera) {
    ...
    report.frustum = {
      tested: true,
      containsForeground: frustum.intersectsBox(unionBox),
