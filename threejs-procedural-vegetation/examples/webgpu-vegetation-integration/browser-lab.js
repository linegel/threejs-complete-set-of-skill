  ...
  const implementation = {
    ...
    async capturePixels(target = "final") {
      ...
      return {
        ...
        sourceByteLength: readback.pixels.byteLength ?? readback.pixels.length,
        compactBytesPerRow: width * 4,
        origin: "bottom-left",
        data: readback.pixels,
        pixels: Array.from(readback.pixels),
      };
    },
    describePipeline() {
    ...
    getMetrics() {
      ...
      const evidence = {
        ...
        deviceIdentitySource: "renderer.backend.device after await renderer.init()",
      ...
      return {
        ...
        initialized: true,
        firstFrameCompleted: frameCount > 0,
        lastFrameError: null,
        ...
        viewport: {
          ...
          dpr: viewport.actualDpr ?? viewport.requestedDpr ?? 1,
        },
        rendererInfo: {
          rendererType: "WebGPURenderer",
          backendType: "WebGPUBackend",
