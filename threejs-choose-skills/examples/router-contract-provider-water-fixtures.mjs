...
function buildPrecipitationGraphLineage( h, route, fields ) {
	...
	const costLedger = {
		...
		workAttribution: [ { workKey: 'provider-precipitation-shared-work-v4', owner: 'route-physics-coordinator', scope: 'shared', targetViewKeys: [], coordinationAdvanceIds: [ coordinationAdvanceId ], stageExecutionPassOrDispatchIds: stageExecutions.map( ( execution ) => execution.executionId ), occurrenceCount: h.evidence( 4, 'occurrence', 'Measured', 'provider precipitation trace v4' ), cpuTime: h.evidence( 0.08, 'millisecond', 'Measured', 'provider precipitation trace v4' ), gpuTime: noResource( 'route-physics-coordinator', 'CPU provider path'), externalLatency: noResource( 'route-physics-coordinator', 'local provider path' ), trafficRecordIds: [], memoryAllocationIds: [ ...hotState.allocations, ...peakTransient.allocations ].map( ( allocation ) => allocation.allocationId ), attributionRule: 'count-shared-once', attributionDigest: 'sha256:provider-precipitation-shared-work-attribution-v4' } ], sharedWorkKeys: [ 'provider-precipitation-shared-work-v4' ], perViewWorkKeys: {}, hotState, peakTransient, migrationOverlap, qualityCostEvidence: [], qualityCostEvidenceResourcesByDigest: {}, qualityMigrationCostEvidence: [], thermalPowerState: { state: 'fixture-not-thermally-significant', duration: h.fixtureDurationSeconds( durationSeconds( coordinationInterval ), 'provider fixture interval' ) }
	};
