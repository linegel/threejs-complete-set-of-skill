...
function buildPrecipitationGraphLineage( h, route, fields ) {
	...
	const costLedger = {
		...
		worstPermittedCatchUpBurst: { triggerAndIntervalDebt: { debt: h.fixtureDurationSeconds( 0, 'provider fixture blocks debt' ) }, executionsDispatchesAndTraffic: { executions: h.evidence( 4, 'execution', 'Derived', 'one admitted provider advance' ), bytes: h.evidence( worstBurstLogicalBytes, 'byte', 'Derived', 'sum of canonical UTF-8 input/output bytes for every admitted stage execution' ) }, latencyMemoryAndErrorGate: { latency: h.evidence( durationSeconds( coordinationInterval ) * 1000, 'millisecond', 'Gated', 'two-tick provider interval' ), memory: h.evidence( peakTransientLogicalBytes, 'byte', 'Gated', 'canonical serialized hot plus transient provider records' ), error: h.evidence( quantityValue( fields.massClosure.maximumResidualKg, 'provider mass residual gate' ), 'kilogram', 'Gated', 'control-volume mass gate' ) } },
		hotBytesReadWrittenPerExecution: Object.fromEntries( stages.map( ( stage ) => [ stage.stageId, { read: h.evidence( perStageLogicalBytes[ stage.stageId ].read, 'byte', 'Derived', 'canonical UTF-8 bytes of exact stage inputs' ), written: h.evidence( perStageLogicalBytes[ stage.stageId ].written, 'byte', 'Derived', 'canonical UTF-8 bytes of exact stage outputs' ) } ] ) ), solverDispatches: [], queueSubmissionsAndPassBreaks: { submissions: h.evidence( 0, 'submission', 'Measured', 'CPU provider trace' ), breaks: h.evidence( 0, 'break', 'Measured', 'CPU provider trace' ) }, dependencyCriticalPaths: [ { path: 'cloud-emission-n-to-rain-transport-n-plus-one-to-atomic-commit', p95: h.evidence( 0.08, 'millisecond', 'Measured', 'provider precipitation trace v4' ) } ],
		...
		workAttribution: [ { workKey: 'provider-precipitation-shared-work-v4', owner: 'route-physics-coordinator', scope: 'shared', targetViewKeys: [], coordinationAdvanceIds: [ coordinationAdvanceId ], stageExecutionPassOrDispatchIds: stageExecutions.map( ( execution ) => execution.executionId ), occurrenceCount: h.evidence( 4, 'occurrence', 'Measured', 'provider precipitation trace v4' ), cpuTime: h.evidence( 0.08, 'millisecond', 'Measured', 'provider precipitation trace v4' ), gpuTime: noResource( 'route-physics-coordinator', 'CPU provider path'), externalLatency: noResource( 'route-physics-coordinator', 'local provider path' ), trafficRecordIds: [], memoryAllocationIds: [ ...hotState.allocations, ...peakTransient.allocations ].map( ( allocation ) => allocation.allocationId ), attributionRule: 'count-shared-once', attributionDigest: 'sha256:provider-precipitation-shared-work-attribution-v4' } ], sharedWorkKeys: [ 'provider-precipitation-shared-work-v4' ], perViewWorkKeys: {}, hotState, peakTransient, migrationOverlap, thermalPowerState: { state: 'fixture-not-thermally-significant', duration: h.fixtureDurationSeconds( durationSeconds( coordinationInterval ), 'provider fixture interval' ) }
	};
