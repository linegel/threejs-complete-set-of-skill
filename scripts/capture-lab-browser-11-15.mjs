...
const PUBLIC_CAPTURE_STATE_FIELDS = Object.freeze([
  'scenario',
  'mode',
  'tier',
  'seed',
...
export async function applyPublicCaptureState(invokeController, state) {
  ...
  const unknown = Object.keys(state).filter((field) => !PUBLIC_CAPTURE_STATE_FIELDS.includes(field));
  const missing = PUBLIC_CAPTURE_STATE_FIELDS.filter((field) => !Object.hasOwn(state, field));
  if (unknown.length > 0) throw new Error(`capture state has unknown fields: ${unknown.join(', ')}`);
  if (missing.length > 0) throw new Error(`capture state omits fields: ${missing.join(', ')}`);
...
export function isBenignCaptureNoise(message) {
  return /favicon\.(ico|svg|png)/i.test(String(message));
}
...
export function assertNoCaptureFailures({ pageErrors = [], consoleErrors = [], requestErrors = [], runtime = null } = {}) {
  const failures = [
    ...
    ...consoleErrors.filter((value) => !isBenignCaptureNoise(value)).map((value) => `console: ${value}`),
    ...requestErrors.filter((value) => !isBenignCaptureNoise(value)).map((value) => `request: ${value}`),
...
async function runBuiltinCapture(session, target) {
  ...
  await applyPublicCaptureState(session.controllerCall, session.lockedState);
...
export async function captureLabBrowser({
  ...
  try {
    ...
    if (closingLab?.sourceHash !== lab.sourceHash) {
      throw new Error(
        `canonical source for ${labId} changed during capture: ${lab.sourceHash} -> ${closingLab?.sourceHash ?? '<missing>'}`,
      );
    }
    if (closingRegistry.buildRevision !== registry.buildRevision) {
      console.warn(`[capture] global buildRevision drifted during ${labId} capture (lab sourceHash stable); continuing`);
    }
