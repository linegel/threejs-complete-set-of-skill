...
const defaultBundle = resolve(root, "artifacts/visual-validation/procedural-district");
...
function validateCorrectnessSession(sessionDir) {
  ...
  if (!existsSync(sessionPath)) return { ok: false, errors: ["missing capture-session.json"] };
  const session = readJson(sessionPath);
  const metrics = session.finalRuntime?.metrics ?? session.runtime?.metrics ?? {};
  const evidence = metrics.rendererBackendEvidence ?? {};
  const errors = [];
  if (session.labId !== "procedural-district") errors.push("capture-session labId mismatch");
