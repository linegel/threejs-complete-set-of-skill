...
const bundleDir = resolve( process.env.LAB_ARTIFACT_DIR ?? resolve( root, 'artifacts/visual-validation/node-selective-bloom' ) );
const requireAllClaims = process.argv.includes( '--require-all-claims' );

if ( ! existsSync( bundleDir ) ) {

	console.error( JSON.stringify( { schemaVersion: 2, labId: 'node-selective-bloom', verdict: 'INSUFFICIENT_EVIDENCE', bundleDir, reason: 'artifact bundle does not exist' }, null, 2 ) );
	process.exit( 1 );
...
console.log( JSON.stringify( { schemaVersion: 2, labId: 'node-selective-bloom', bundleDir, structuralVerdict: errors.length === 0 ? 'PASS' : 'FAIL', requireAllClaims, errors }, null, 2 ) );
