...
const classification = Object.freeze(demoId.includes("-generated-")
  ? {
      kind: "generated-asset-demo",
      label: "Generated asset preview",
      limitation: "This secondary page previews generated texture inputs only. It does not prove the skill's canonical native-WebGPU mechanism, performance, or acceptance.",
    }
  : {
      kind: "proxy-demo",
      label: "Concept proxy",
      limitation: "This secondary teaching surface does not prove the skill's canonical native-WebGPU mechanism, performance, or acceptance.",
    });

const authoredCanonicalHref = evidenceEl.getAttribute("href");
const canonicalHref = authoredCanonicalHref
  ? new URL(authoredCanonicalHref, document.baseURI).href
  : new URL(config.evidenceHref ?? `../../skills/${config.skill.slice(1)}.html`, import.meta.url).href;
const classificationEl = document.createElement("aside");
classificationEl.className = "classification";
classificationEl.dataset.kind = classification.kind;
const classificationLabel = document.createElement("strong");
classificationLabel.textContent = classification.label;
const classificationLimitation = document.createElement("span");
classificationLimitation.textContent = classification.limitation;
const classificationLink = document.createElement("a");
classificationLink.href = canonicalHref;
classificationLink.textContent = "Open canonical lab or contract";
classificationEl.append(classificationLabel, classificationLimitation, classificationLink);
claimEl.before(classificationEl);

titleEl.textContent = config.title;
skillEl.textContent = config.skill;
claimEl.textContent = config.claim;
evidenceEl.href = canonicalHref;
evidenceEl.textContent = "Canonical lab or contract";
...
function createModeButtons(sceneApi) {
  ...
  evidence.textContent = "Canonical lab or contract";
...
async function init() {
  ...
  state.ready = true;
  setStatus(`${classification.label} ready on WebGPU. Secondary presentation only.`, "ready");
