...
const result = validateEvidenceBundle(artifactDir, { requireRequiredClaimsPass: true });
const errors = [...result.errors];
const evidence = result.json["evidence-manifest.json"];
if (evidence?.labId !== lab.id) errors.push("evidence-manifest.json labId mismatch");
if (evidence?.sourceHash !== lab.sourceHash) errors.push("evidence-manifest.json sourceHash does not match the shared canonical-source hash");
if (evidence?.status !== "accepted") errors.push("evidence-manifest.json status must be accepted");
...
if (!existsSync(resolve(artifactDir, "capture-session.json"))) errors.push("missing shared capture-session.json");
  ...
  if (captureSession.sourceHash !== lab.sourceHash) errors.push("capture-session.json sourceHash mismatch");
  if (captureSession.runtime?.metrics?.opaqueInputsReachable !== true) errors.push("capture session did not prove opaque optical inputs reachable");
}
...
console.log(JSON.stringify({ pass: true, verdict: "PASS", labId: lab.id, artifactDir, sourceHash: lab.sourceHash }, null, 2));
