...
`npm run capture` starts the immutable, content-addressed server and prints the exact route-matrix URL plus three Codex in-app Browser correctness URLs. It never launches Playwright, Chrome, or another browser. Each subject URL auto-runs a bounded 21-readback segment: 16 presentation states and 5 target masks at exactly 1200×800, DPR 1. The page retains the renderer's actual transport bytes and an independently allocated, fully padded normalization using the recorded integer row stride. It then validates those bytes, emits a DOM-visible 21/21 summary, and exposes a TAR download capped at 192 MiB. Collecting one subject per page bounds live retained byte arrays instead of holding all 63 readbacks at once. Stop the server with Ctrl-C only after saving all three TARs.

After saving all three TAR files, run `npm run capture:import -- --segment <lamp.tar> --segment <bonsai.tar> --segment <teapot.tar> --output <absent-directory>`. The offline importer bounds and opens only nonsymlink regular inputs, requires canonical ustar headers/padding/two-block termination, recomputes every subject-document digest, and pins source hash, build revision, and Three.js revision to the current generated closure. It rejects missing, duplicate, colliding, cross-source, reordered, hash-mismatched, stride-mismatched, flat presentation, and non-binary mask evidence before creating output. The output tree must not exist; files use exclusive creation and every directory component is rechecked against symlink traversal. The self-contained result retains the exact three input TARs and exact three subject documents, reconstructs compact RGBA only from each normalized artifact's recorded integer row stride, writes 48 presentation PNGs plus 15 mask PNGs, derives all eight standard contact sheets including `final.design.png`, `diagnostics.mosaic.png`, and temporal endpoints, preserves transport and normalized byte artifacts, and writes `correctness-in-app-import.json`. `--check` performs the same validation and PNG/contact-sheet construction without touching output.
...
They are not solver-ready collider or mass-property records. This lab does not own the host scale or context version, registered live simulation frame/origin/transform revision, committed pose signal, validity interval, collision/contact owner, density, mass, center of mass, inertia tensor, pair law, contact manifold, constraint solve, fracture solve, or solver integration. The visible animation is procedural kinematic presentation, not a physics solution. Accordingly, the HUD reports collider handoffs as authoring inputs and the host simulation handoff as blocked until a separate adapter supplies and validates that missing evidence.
