...
if (!existsSync(sessionPath)) {
  ...
  if (metrics?.nativeWebGPU !== true || metrics?.backend !== "webgpu") errors.push("capture session did not prove native WebGPU");
  if (metrics?.initialized !== true || metrics?.firstFrameCompleted !== true) errors.push("capture session did not complete its first frame");
  if (metrics?.lastFrameError !== null) errors.push("capture session recorded a frame error");
  if (pipeline?.owner !== "WebGPURenderer" || pipeline?.sceneRendersPerFrame !== 1) errors.push("capture session did not preserve single-renderer/single-scene ownership");
  if (pipeline?.finalOutputOwner !== "renderer" || pipeline?.outputColorSpace !== "srgb") errors.push("capture session output ownership or color space drifted");
  if (session.hookResult?.evidenceStatus !== "INCOMPLETE") errors.push("capture hook must preserve the incomplete claim boundary");
  if (captures.length !== requiredImages.length) errors.push(`expected ${requiredImages.length} captures, received ${captures.length}`);
  for (const capture of captures) {
    if (!requiredImages.includes(capture.filename)) errors.push(`unexpected capture ${capture.filename}`);
    if (capture.format !== "rgba8" || capture.colorEncoding !== "srgb") errors.push(`${capture.filename} is not explicit color-managed RGBA8`);
    if (capture.bytesPerRow !== capture.width * 4) errors.push(`${capture.filename} compact row stride mismatch`);
    if (capture.sourceBytesPerRow < capture.bytesPerRow || capture.sourceBytesPerRow % 256 !== 0) errors.push(`${capture.filename} source row stride is not 256-byte aligned`);
    ...
    if (capture.sourceLayout !== "padded") errors.push(`${capture.filename} did not preserve padded readback evidence`);
  }
}
